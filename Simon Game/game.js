...
var checkAnswer = function (currentLevel) {
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    ...
    if (userClickedPattern.length === gamePattern.length) {
      //agar userclickedpattern or gamepattern li length same h matlab dono me no. of elements of arrays same h toh ye cond true h
...
var nextSequence = function () {
  ...
  $("#" + randomChosenColor)
    .fadeIn(1000)
    .fadeOut(1000)
    .fadeIn(1000); // jo bhi random color aya h usme ham ne fadein fade out vali property laga di h har bar yeh function chalne pr ek naya array generate hoga or usme sirf ek color hoga or usme hi fadein or fadeout vali property lag jaygi

  playSound(randomChosenColor);
  animatePress(randomChosenColor);

  game_level = level;
  $(".game_level").text( level);
